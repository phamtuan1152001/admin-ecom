...
import UploadImage from '../Create/components/Upload'
...
function UpdateProduct() {
  ...
  // console.log("location", productInfo);
  ...
  const fetchGetDetailProduct = async () => {
    try {
      setLoading(true)
      const req = {
        cateSlug: productInfo?.categories?.slug,
        slug: productInfo?.slug
      }
      const res = await getDetailProduct(req)
      if (res?.retCode === SUCCESS) {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const res = await updateDetailProduct(req);
      // console.log("req", res);
      if (res?.retCode === SUCCESS) {
